...
import { useOutletContext } from "react-router"; // 引入套版 Context
...
function Articles() {
  ...
  const AVATAR_OPTIONS = [
    { label: "頭貼1", value: "https://storage.googleapis.com/vue-course-api.appspot.com/leafandhome/1770091044569.png" },
    { label: "頭貼2", value: "https://storage.googleapis.com/vue-course-api.appspot.com/leafandhome/1770091120517.png" },
    { label: "頭貼3", value: "https://storage.googleapis.com/vue-course-api.appspot.com/leafandhome/1770091144329.png" },
    { label: "頭貼4", value: "https://storage.googleapis.com/vue-course-api.appspot.com/leafandhome/1770091165196.png" },
  ];

  // --- 🔄 初始化 Editor.js 與 監聽 Token ---
  useEffect(() => {
    if (token) {
      ...
      const editor = new EditorJS({
        ...
        tools: {
          ...
          image: { 
            class: UrlImage,
            config: {
              apiConfig: { API_BASE, API_PATH } // 傳入 API 資訊給插件
            }
          },
        },
        onReady: () => {
  ...
  const handleSave = async () => {
    ...
    const convertedBlocks = editorData.blocks
      .map((block) => {
        if (block.type === "paragraph")
          return { type: "paragraph", content: block.data.text.replace(/\n/g, "<br>") };
        if (block.type === "header")
          return { type: "heading", level: block.data.level, content: block.data.text.replace(/\n/g, "<br>") };
        if (block.type === "image")
    ...
    convertedBlocks.push({ type: "relatedProducts", title: "與植物相遇的傳送門", products: relatedProducts });
  ...
  const handleEdit = async (article) => {
    try {
      ...
      const editorBlocks = blocks
        ...
        .map((b) => {
          if (b.type === "paragraph")
            return { type: "paragraph", data: { text: b.content.replace(/<br\s*\/?>/gi, "\n") } };
          if (b.type === "heading")
            return { type: "header", data: { text: b.content.replace(/<br\s*\/?>/gi, "\n"), level: b.level } };
          if (b.type === "image")
            ...
            <div className="col-md-6">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setComments([...comments, { userName: "", content: "", avatarType: randomAvatar }]);
                }}
              >
                + 新增留言
              </button>
          ...
          <div className="mt-4">
            {editId ? (
              <div className="d-flex gap-3">
                ...
                <button className="btn btn-outline-danger btn-lg fw-bold shadow py-3 px-5" onClick={() => window.confirm("確定取消？") && resetForm()}>取消編輯</button>
              </div>
            ) : (
            ...
            <div key={a.id} className="list-group-item d-flex justify-content-between align-items-center mb-2 rounded border-0 shadow-sm p-3 bg-white">
              ...
              <div className="btn-group gap-2">
                ...
                <button
                  ...
                  onClick={() => window.confirm("確定刪除？") && axios.delete(`${API_BASE}/api/${API_PATH}/admin/article/${a.id}`).then(fetchArticles)}
                >
                  刪除
                </button>
              </div>
